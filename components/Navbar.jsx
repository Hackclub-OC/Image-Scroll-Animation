    ...
    <nav className="fixed max-w-xl border border-black dark:border-white transition-colors duration-500 icon-transition w-full rounded-full mx-auto top-4 left-0 right-0 z-50 bg-white/80 dark:bg-black/80 text-black dark:text-white p-2 flex items-center justify-between">
      ...
      <div className="flex items-center">
        <Tooltip title="Toggle View" placement="bottom">
          ...
          >
            {isGridView ? (
              <ContainerIcon className="w-6 h-6 text-gray-800 dark:text-white transition-colors duration-300 icon-transition" />
            ) : (
              <SectionIcon className="w-6 h-6 text-gray-800 dark:text-white transition-colors duration-300 icon-transition" />
            )}
          </button>
        </Tooltip>

        <Tooltip title="View Code" placement="bottom">
        <a
